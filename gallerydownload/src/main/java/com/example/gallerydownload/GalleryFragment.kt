...
class GalleryFragment : Fragment() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when(item.itemId){//刷新子菜单点击事件
            R.id.menuGalleryRefresh->{
                swipeLayoutGallery.isRefreshing=true//显示加载进度条
                //模拟加载延时，设置1000毫秒后执行刷新
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        recyclerViewGallery.apply {
            ...
            //设置recyclerView的布局管理器；每行显示两个，垂直显示
            layoutManager = StaggeredGridLayoutManager(2,StaggeredGridLayoutManager.VERTICAL)
        }
        //添加观察者，监控数据变化
        galleryViewModel.photoListLive.observe(requireActivity(),Observer{
            galleryAdapter.submitList(it)//将新的数据加载到适配器
            swipeLayoutGallery.isRefreshing=false//变化完成，停止加载
        })
        //进行第一次的数据加载
